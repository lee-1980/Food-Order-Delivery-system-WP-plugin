 ...
 * @since 1.0
 */
class pl8app_Payments_Query extends pl8app_Stats {

	/**
	 * The args to pass to the pl8app_get_payments() query
	 *
	 * @var array
	 * @since 1.0
	 */
	...
	public function __construct( $args = array() ) {
		$defaults = array(
			...
			'post_type'       => array( 'pl8app_payment' ),
			'start_date'      => false,
			'end_date'        => false,
			'service_date'	  => false,
			'number'          => 20,
			...
			'user'            => null,
			'customer'        => null,
			'status'          => pl8app_get_payment_status_keys(),
			'meta_key'        => null,
			'year'            => null,
			'month'           => null,
			'day'             => null,
	 ...
	 *
	 * @since 1.0
	 * @return pl8app_Payment[]
	 */
	public function get_payments() {

	    // Set the Last Order ID to get the new Order Lists
        add_filter( 'posts_where', array( $this, 'payments_in_id_where' ) );
		...
		do_action( 'pl8app_pre_get_payments', $this );
		...
		if ( $query->have_posts() ) {
			...
			while ( $query->have_posts() ) {
				...
				$this->payments[] = apply_filters( 'pl8app_payment', $payment, $payment_id, $this );
			}

		}

		add_action( 'pl8app_post_get_payments', array( $this, 'date_filter_post' ) );
        remove_filter( 'posts_where', array( $this, 'payments_in_id_where' ) );
		do_action( 'pl8app_post_get_payments', $this );
    ...
    /**
     * If the Last ID is assigned, get the new Orders based on that.
     */
	public function payments_in_id_where($where = ''){

        global $wpdb;

        if(!empty($this->args['last_old_id'])){
            $where .= "AND $wpdb->posts.ID > '{$this->args['last_old_id']}'";
        }
        return $where;
    }
	/**
	...
	public function orderby() {
		switch ( $this->args['orderby'] ) {
			case 'amount' :
				...
				$this->__set( 'meta_key', '_pl8app_payment_total' );
			break;
			default :
	...
	public function user() {
		...
		if ( is_numeric( $this->args['user'] ) ) {
			$user_key = '_pl8app_payment_user_id';
		} else {
			$user_key = '_pl8app_payment_user_email';
		}
	...
	public function customer() {
		...
		$this->__set( 'meta_query', array(
			'key'   => '_pl8app_payment_customer_id',
    ...
    /**
     * Specific the Order Status
     */
	public function order_statue(){
        if ( is_null( $this->args['order_statue'] ) ) {
            return;
        }
        $this->__set( 'meta_query', array(
            'key'   => '_order_status',
            'value' => $this->args['order_statue']
        ) );
    }
	...
	public function search() {
		...
		if ( ! empty( $this->args['search_in_notes'] ) ) {
			...
			$notes = pl8app_get_payment_notes( 0, $search );
			...
			$key = $is_email ? '_pl8app_payment_user_email' : '_pl8app_payment_purchase_key';
			$search_meta = array(
				'key'     => $key,
				'value'   => $search,
				'compare' => 'LIKE'
			...
			pl8app_get_option( 'enable_sequential' ) &&
			(
				false !== strpos( $search, pl8app_get_option( 'sequential_prefix' ) ) ||
				false !== strpos( $search, pl8app_get_option( 'sequential_postfix' ) )
			)
		) {
			...
			if( is_object( $post ) && $post->post_type == 'pl8app_payment' ) {
	...
	public function menuitem() {
		...
		if ( empty( $this->args['menuitem'] ) )
			return;

		global $pl8app_logs;

		$args = array(
			'post_parent'            => $this->args['menuitem'],
			'log_type'               => 'sale',
		...
		if ( is_array( $this->args['menuitem'] ) ) {
			unset( $args['post_parent'] );
			$args['post_parent__in'] = $this->args['menuitem'];
		}

		$sales = $pl8app_logs->get_connected_logs( $args );
		...
		if ( ! empty( $sales ) ) {
			...
			foreach ( $sales as $sale ) {
				$payments[] = get_post_meta( $sale, '_pl8app_log_payment_id', true );
			}
