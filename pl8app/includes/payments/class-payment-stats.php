 ...
 * @since 1.0
 */
class pl8app_Payment_Stats extends pl8app_Stats {
	/**
	 * Retrieve sale stats
	 *
	 * @since 1.0
	 *
	 * @param int          $menuitem_id The menuitem product to retrieve stats for. If false, gets stats for all products
	 ...
	 */
	public function get_sales( $menuitem_id = 0, $start_date = false, $end_date = false, $status = 'publish' ) {
		...
		if ( empty( $menuitem_id ) ) {
			// Global sale stats
			add_filter( 'pl8app_count_payments_where', array( $this, 'count_where' ) );

			if ( is_array( $status ) ) {
				$count = 0;
				foreach ( $status as $payment_status ) {
					$count += pl8app_count_payments()->$payment_status;
				}
			} else {
				$count = pl8app_count_payments()->$status;
			}

			remove_filter( 'pl8app_count_payments_where', array( $this, 'count_where' ) );
		} else {
			...
			$count = $pl8app_logs->get_log_count( $menuitem_id, 'sale' );
	 ...
	 * @return float|int Total amount of sales based on the passed arguments.
	 */
	public function get_earnings( $menuitem_id = 0, $start_date = false, $end_date = false, $include_taxes = true ) {
		global $wpdb;
		...
		if ( empty( $menuitem_id ) ) {
			...
			$args = array(
				...
				'pl8app_transient_type'     => 'pl8app_earnings', // This is not a valid query arg, but is used for cache keying
				'include_taxes'          => $include_taxes,
			);

			$args   = apply_filters( 'pl8app_stats_earnings_args', $args );
			...
			if ( ! isset( $cached[ $key ] ) ) {
				...
				if ( $sales ) {
					...
					$total_earnings = apply_filters( 'pl8app_payment_stats_earnings_total', $total_earnings, $sales, $args );
			...
			if ( ! isset( $cached[ $key ] ) ) {
				...
				if ( $log_ids ) {
					...
					$payment_ids = $wpdb->get_col( "SELECT DISTINCT meta_value FROM $wpdb->postmeta WHERE meta_key = '_pl8app_log_payment_id' AND post_id IN ($log_ids);" );

					foreach ( $payment_ids as $payment_id ) {
						$items = pl8app_get_payment_meta_cart_details( $payment_id );
						...
						foreach ( $items as $cart_key => $item ) {
							...
							$earnings = apply_filters( 'pl8app_payment_stats_item_earnings', $earnings, $payment_id, $cart_key, $item );

							if ( ! $include_taxes ) {
								$earnings -= pl8app_get_payment_item_tax( $payment_id, $cart_key );
							}
						}
					}
				}
		...
		return round( $result, pl8app_currency_decimal_filter() );
	}
	 ...
	 *
	 * @return array List of menuitem IDs that are best selling
	 */
	public function get_best_selling( $number = 10 ) {
		global $wpdb;

		$menuitems = $wpdb->get_results( $wpdb->prepare(
			"SELECT post_id as menuitem_id, max(meta_value) as sales
			 FROM $wpdb->postmeta
			 WHERE meta_key='_pl8app_menuitem_sales' AND meta_value > 0
	...
	public function get_sales_by_range( $range = 'today', $day_by_day = false, $start_date = false, $end_date = false, $status = 'publish' ) {
		...
		if ( false === $sales || ! $this->is_cacheable( $range ) ) {
			...
			$statuses = apply_filters( 'pl8app_payment_stats_post_statuses', array( 'publish', 'revoked' ) );
			$statuses = "'" . implode( "', '", $statuses ) . "'";

			$sales = $wpdb->get_results( $wpdb->prepare(
				"SELECT $select
				 FROM {$wpdb->posts} AS posts
				 WHERE posts.post_type IN ('pl8app_payment')
		...
		if ( false === $sales || ! $this->is_cacheable( $range ) ) {
			...
			$earnings = $wpdb->get_results( $wpdb->prepare(
				"SELECT SUM(meta_value) AS total, $select
				 ...
				 WHERE posts.post_type IN ('pl8app_payment')
				 AND {$wpdb->postmeta}.meta_key = '_pl8app_payment_total'
				 AND posts.post_date >= %s
				 AND posts.post_date < %s
				 AND posts.post_status IN ($statuses)
				 GROUP BY $grouping
			...
			if ( ! $include_taxes ) {
				$taxes = $wpdb->get_results( $wpdb->prepare(
					"SELECT SUM(meta_value) AS tax, $select
					 ...
					 INNER JOIN {$wpdb->postmeta} ON posts.ID = {$wpdb->postmeta}.post_ID
					 WHERE posts.post_type IN ('pl8app_payment')
					 AND {$wpdb->postmeta}.meta_key = '_pl8app_payment_tax'
					 AND posts.post_date >= %s
					 AND posts.post_date < %s
					 AND posts.post_status IN ($statuses)
					 GROUP BY $grouping
