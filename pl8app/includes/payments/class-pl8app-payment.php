 ...
 * Payments
 *
 * This class is for working with payments in pl8app.
 *
 * @package     pl8app
...
class pl8app_Payment {
	...
	/**
	 * Array of menuitems for this payment
	 *
	 * @since  1.0.0
	 * @var array
	 */
  ...
  protected $delivery_date = '';
	...
	/**
	 * Allows the files for this payment to be menuitemed unlimited times (when menuitem limits are enabled)
	 *
	 * @since  1.0.0
	 * @var boolean
	 */
	protected $has_unlimited_menuitems = false;
	...
	/**
	 * Setup the pl8app Payments class
	 *
	 * @since  1.0.0
	...
	public function __set( $key, $value ) {
		$ignore = array( 'menuitems', 'cart_details', 'fees', '_ID' );
	...
	private function setup_payment( $payment_id ) {
		...
		if( 'pl8app_payment' !== $payment->post_type ) {
			return false;
		}

		// Allow extensions to perform actions before the payment is loaded
		do_action( 'pl8app_pre_setup_payment', $this, $payment_id );
		...
		$all_payment_statuses  = pl8app_get_payment_statuses();
		...
		$this->menuitems       = $this->setup_menuitems();
		...
		$this->has_unlimited_menuitems = $this->setup_has_unlimited();

		// Allow extensions to add items to this object via hook
		do_action( 'pl8app_setup_payment', $this, $payment_id );
	...
	private function insert_payment() {
		...
		if ( empty( $this->key ) ) {
			...
			$this->key = strtolower( md5( $this->email . date( 'Y-m-d H:i:s' ) . $auth_key . uniqid( 'pl8app', true ) ) );  // Unique key
			$this->pending['key'] = $this->key;
		}

		if ( empty( $this->ip ) ) {

			$this->ip = pl8app_get_ip();
		...
		$args = apply_filters( 'pl8app_insert_payment_args', array(
		...
		if ( ! empty( $payment_id ) ) {
			...
			/**
			 * This run of the pl8app_payment_meta filter is for backwards compatibility purposes. The filter will also run in the pl8app_Payment::save
			 ...
			 * by pl8app_insert_payment().
			 *
			 */
			$this->payment_meta = apply_filters( 'pl8app_payment_meta', $this->payment_meta, $payment_data );

			$this->delivery_address = apply_filters( 'pl8app_delivery_address_meta', $this->delivery_address );

			$this->order_note = apply_filters( 'pl8app_order_note_meta', $this->order_note );
			...
			if ( pl8app_get_option( 'enable_sequential' ) ) {
				$number       = pl8app_get_next_payment_number();
				$this->number = pl8app_format_payment_number( $number );
				$this->update_meta( '_pl8app_payment_number', $this->number );
				update_option( 'pl8app_last_payment_number', $number );
			}

			$this->update_meta( '_pl8app_payment_meta', $this->payment_meta );

			$this->update_meta( '_pl8app_delivery_address', $this->delivery_address );

			$this->update_meta( '_pl8app_order_note', $this->order_note );
	...
	public function save() {
		...
		if ( ! empty( $this->pending ) ) {
			...
			foreach ( $this->pending as $key => $value ) {
				switch( $key ) {
					...
					case 'menuitems':
						...
						// Update totals for pending menuitems
						foreach ( $this->pending[ $key ] as $item ) {
							...
							switch( $item['action'] ) {
								...
								case 'add':
									...
									if ( 'publish' === $this->status || 'complete' === $this->status || 'revoked' === $this->status ) {
										...
										while ( $y < $item['quantity'] ) {
											pl8app_record_sale_in_log( $item['id'], $this->ID, $price_id, $log_date );
											$y++;
										}
										...
										$menuitem->increase_sales( $item['quantity'] );
										$menuitem->increase_earnings( $increase_earnings );
								...
								case 'remove':
									...
									$found_logs = $pl8app_logs->get_connected_logs( $log_args );
									...
									if ( 'publish' === $this->status || 'complete' === $this->status || 'revoked' === $this->status ) {
										$menuitem = new pl8app_Menuitem( $item['id'] );
										$menuitem->decrease_sales( $item['quantity'] );
										...
										$menuitem->decrease_earnings( $decrease_amount );
								...
								case 'modify':
									...
									if ( 'publish' === $this->status || 'complete' === $this->status || 'revoked' === $this->status ) {
										...
										if ( $item['previous_data']['quantity'] != $item['quantity'] ) {
											...
											$existing_logs = $pl8app_logs->get_connected_logs( $log_args );
					...
					case 'gateway':
						$this->update_meta( '_pl8app_payment_gateway', $this->gateway );
						break;

					case 'mode':
						$this->update_meta( '_pl8app_payment_mode', $this->mode );
						break;

					case 'transaction_id':
						$this->update_meta( '_pl8app_payment_transaction_id', $this->transaction_id );
						break;

					case 'ip':

						$this->update_meta( '_pl8app_payment_user_ip', $this->ip );
						break;

					case 'customer_id':
						$this->update_meta( '_pl8app_payment_customer_id', $this->customer_id );
						$customer = new pl8app_Customer( $this->customer_id );
						$customer->attach_payment( $this->ID, false );
						break;

					case 'user_id':
						$this->update_meta( '_pl8app_payment_user_id', $this->user_id );
					...
					case 'delivery_type':
						$service_type = pl8app_selected_service();
						$this->update_meta( '_pl8app_delivery_type', $service_type );
						break;

					case 'delivery_time':
						$delivery_time = isset($_COOKIE['service_time']) ? $_COOKIE['service_time'] : '';
						$this->update_meta( '_pl8app_delivery_time', $delivery_time );
						break;

					case 'delivery_date':
    					$service_date = isset( $_COOKIE['service_date'] ) ? $_COOKIE['service_date'] : '';
      					$this->update_meta( '_pl8app_delivery_date', $service_date );
      					break;
					...
					case 'email':
						...
						$this->update_meta( '_pl8app_payment_user_email', $this->email );
						break;

					case 'key':
						$this->update_meta( '_pl8app_payment_purchase_key', $this->key );
						break;

					case 'tax_rate':
						$this->update_meta( '_pl8app_payment_tax_rate', $this->tax_rate );
						break;
					...
					case 'completed_date':
						$this->update_meta( '_pl8app_completed_date', $this->completed_date );
						break;

					case 'has_unlimited_menuitems':
						$this->update_meta( '_pl8app_payment_unlimited_menuitems', $this->has_unlimited_menuitems );
						break;
					...
					default:
						 ...
						 */
						do_action( 'pl8app_payment_save', $this, $key );
						break;
				}
			}
			...
			if ( ! $this->in_process() ) {
				...
				if ( $total_change < 0 ) {
					...
					pl8app_decrease_total_earnings( $total_change );

				} else if (  $total_change > 0 ) {

					// Increase the customer's purchase stats
					$customer->increase_value( $total_change );
					pl8app_increase_total_earnings( $total_change );

				}
			}

			$this->update_meta( '_pl8app_payment_total', $this->total );
			$this->update_meta( '_pl8app_payment_tax', $this->tax );
			...
			// Do some merging of user_info before we merge it all, to honor the pl8app_payment_meta filter
			...
			$merged_meta = apply_filters( 'pl8app_payment_meta', $merged_meta, $payment_data );

			// Only save the payment meta if it's changed
			if ( md5( serialize( $meta ) ) !== md5( serialize( $merged_meta) ) ) {
				$updated     = $this->update_meta( '_pl8app_payment_meta', $merged_meta );
				if ( false !== $updated ) {
					$saved = true;
				}
			}
		...
		if ( true === $saved ) {
			 ...
			 */
			do_action( 'pl8app_payment_saved', $this->ID, $this );
		}

		/**
		 * Update the payment in the object cache
		 */
		$cache_key = md5( 'pl8app_payment' . $this->ID );
	...
	/**
	 * Add a menuitem to a given payment
	 *
	 * @since  1.0.0
	 * @param int   $menuitem_id The menuitem to add
	 * @param array $args Other arguments to pass to the function
	 * @param array $options List of menuitem options
	 * @return bool True when successful, false otherwise
	 */
	public function add_menuitem( $menuitem_id = 0, $args = array(), $options = array() ) {

		$menuitem = new pl8app_Menuitem( $menuitem_id );

		// Bail if this post isn't a menuitem
		if( ! $menuitem || $menuitem->post_type !== 'menuitem' ) {
			return false;
		}
		...
		$args = wp_parse_args( apply_filters( 'pl8app_payment_add_menuitem_args', $args, $menuitem->ID ), $defaults );
		...
		if( false !== $args['item_price'] ) {
			...
			if( pl8app_has_variable_prices( $menuitem->ID ) ) {
				$prices = get_post_meta( $menuitem->ID, 'pl8app_variable_prices', true );

				if( $args['price_id'] && array_key_exists( $args['price_id'], (array) $prices ) ) {
					$item_price = $prices[$args['price_id']]['amount'];
				} else {
					$item_price       = pl8app_get_lowest_price_option( $menuitem->ID );
					$args['price_id'] = pl8app_get_lowest_price_id( $menuitem->ID );
				}
			} else {
				$item_price = pl8app_get_menuitem_price( $menuitem->ID );
			}
		}

		// Sanitizing the price here so we don't have a dozen calls later
		$item_price = pl8app_sanitize_amount( $item_price );
		$quantity   = absint( $args['quantity'] );
		$amount     = round( $item_price * $quantity, pl8app_currency_decimal_filter() );
		...
		$new_menuitem['options'] = $options;

		$this->menuitems[] = $new_menuitem;
        ...
        // Tax rate, Tax name
        if(isset($args['tax_key']) && pl8app_use_taxes()){
            $tax_object = pl8app_get_tax_rates($args['tax_key']);
            $cart_item_details['tax_rate'] = isset($tax_object['name'])?$tax_object['rate']: 0.00;
            $cart_item_details['tax_name'] = isset($tax_object['name'])?$tax_object['name']: 'Zero';
            $cart_item_details['tax_desc'] = isset($tax_object['name'])?$tax_object['desc']: 'No tax';
        }

        $this->cart_details[] = $cart_item_details;

		$added_menuitem = end( $this->cart_details );
		$added_menuitem['action']  = 'add';

		$this->pending['menuitems'][] = $added_menuitem;
	...
	/**
	 * Remove a menuitem from the payment
	 *
	 * @since  1.0.0
	 * @param  int   $menuitem_id The menuitem ID to remove
	 * @param  array $args        Arguments to pass to identify (quantity, amount, price_id)
	 * @return bool               If the item was removed or not
	 */
	public function remove_menuitem( $menuitem_id, $args = array() ) {
		...
		/**
		 * Bail if this post isn't a menuitem post type.
		 *
		 * We need to allow this to process though for a missing post ID, in case it's a menuitem that was deleted.
		 */
		if( ! empty( $menuitem->ID ) && $menuitem->post_type !== 'menuitem' ) {
			return false;
		}

		foreach ( $this->menuitems as $key => $item ) {

			if ( (int) $menuitem_id !== (int) $item['id'] ) {
				continue;
			}
			...
			if ( false !== $args['price_id'] ) {
				...
				if ( ! empty( $cart_item ) ) {
					...
					// If the cart index item isn't the same menuitem ID, don't remove it
			...
			$item_quantity = $this->menuitems[ $key ]['quantity'];

			if ( $item_quantity > $args['quantity'] ) {

				$this->menuitems[ $key ]['quantity'] -= $args['quantity'];
				break;
		...
		if ( false === $args['cart_index'] ) {
			...
			if ( (int) $this->cart_details[ $cart_index ]['id'] !== (int) $menuitem_id ) {
		...
		if ( $orig_quantity > $args['quantity'] ) {
			...
			$total_reduced = round( $item_price * $args['quantity'], pl8app_currency_decimal_filter() );
			$tax_reduced   = round( ( $tax / $orig_quantity ) * $args['quantity'], pl8app_currency_decimal_filter() );
		...
		$this->pending['menuitems'][] = $pending_args;
	...
	public function modify_cart_item( $cart_index = false, $args = array(), $addon_data = array() ) {
		...
		$allowed_items = apply_filters( 'pl8app_allowed_cart_item_modifications', array(
		...
		$merged_item['item_price'] = pl8app_sanitize_amount( $merged_item['item_price'] );
		...
		$modified_menuitem                  = $merged_item;
		$modified_menuitem['action']        = 'modify';
		$modified_menuitem['previous_data'] = $current_args;

		$this->pending['menuitems'][] = $modified_menuitem;

		if ( $new_subtotal > $current_args['subtotal'] ) {
			$this->increase_subtotal( ( $new_subtotal - $modified_menuitem['discount'] ) - $current_args['subtotal'] );
		} else {
			$this->decrease_subtotal( $current_args['subtotal'] - ( $new_subtotal - $modified_menuitem['discount'] ) );
		}

		if ( $modified_menuitem['tax'] > $current_args['tax'] ) {
			$this->increase_tax( $modified_menuitem['tax'] - $current_args['tax'] );
		} else {
			$this->increase_tax( $current_args['tax'] - $modified_menuitem['tax'] );
		}
	...
	public function remove_fee_by( $key, $value, $global = false ) {
		...
		$allowed_fee_keys = apply_filters( 'pl8app_payment_fee_keys', array(
	...
	public function get_fees( $type = 'all' ) {
		...
		return apply_filters( 'pl8app_get_payment_fees', $fees, $this->ID, $this );
	}
	...
	public function add_note( $note = false ) {
		...
		pl8app_insert_payment_note( $this->ID, esc_html( $note ) );
	}
	...
	public function update_status( $status = false ) {
		...
		$do_change = apply_filters( 'pl8app_should_update_payment_status', true, $this->ID, $status, $old_status );

		$updated = false;

		if ( $do_change ) {

			do_action( 'pl8app_before_payment_status_change', $this->ID, $status, $old_status );

			$update_fields = array( 'ID' => $this->ID, 'post_status' => $status, 'edit_date' => current_time( 'mysql' ) );

			$updated = wp_update_post( apply_filters( 'pl8app_update_payment_status_fields', $update_fields ) );
			...
			do_action( 'pl8app_update_payment_status', $this->ID, $status, $old_status );
	 ...
	 */
	public function get_meta( $meta_key = '_pl8app_payment_meta', $single = true ) {

		$meta = get_post_meta( $this->ID, $meta_key, $single );
		if ( $meta_key === '_pl8app_payment_meta' ) {
			...
			if ( is_array( $meta ) && isset( $meta[0] ) ) {
				...
				update_post_meta( $this->ID, '_pl8app_payment_meta', $meta );
			}

			// Payment meta was simplified in pl8app v1.5, so these are here for backwards compatibility
			if ( empty( $meta['key'] ) ) {
		...
		$meta = apply_filters( 'pl8app_get_payment_meta_' . $meta_key, $meta, $this->ID );
		...
		return apply_filters( 'pl8app_get_payment_meta', $meta, $this->ID, $meta_key );
	}
	...
	public function update_meta( $meta_key = '', $meta_value = '', $prev_value = '' ) {
		...
		if( '_pl8app_payment_purchase_key' == $meta_key ) {

			$current_meta = $this->get_meta();
			$current_meta[ 'key' ] = $meta_value;
			update_post_meta( $this->ID, '_pl8app_payment_meta', $current_meta );
			...
			$meta_key     = '_pl8app_payment_meta';
			$meta_value   = $current_meta;

		} else if ( $meta_key == 'email' || $meta_key == '_pl8app_payment_user_email' ) {

			$meta_value = apply_filters( 'pl8app_pl8app_update_payment_meta_' . $meta_key, $meta_value, $this->ID );
			update_post_meta( $this->ID, '_pl8app_payment_user_email', $meta_value );

			$current_meta = $this->get_meta( '_pl8app_payment_meta' );
		...
		$meta_value = apply_filters( 'pl8app_update_payment_meta_' . $meta_key, $meta_value, $this->ID );
	...
	public function is_recoverable() {
		...
		$recoverable_statuses = apply_filters( 'pl8app_recoverable_payment_statuses', array( 'pending', 'abandoned', 'failed' ) );
	...
	public function get_recovery_url() {
		...
		$recovery_url = add_query_arg( array( 'pl8app_action' => 'recover_payment', 'payment_id' => $this->ID ), pl8app_get_checkout_uri() );

		return apply_filters( 'pl8app_payment_recovery_url', $recovery_url, $this );
	}
	...
	private function process_refund() {
		...
		$process_refund = apply_filters( 'pl8app_should_process_refund', $process_refund, $this );

		if ( false === $process_refund ) {
			return;
		}

		do_action( 'pl8app_pre_refund_payment', $this );

		$decrease_store_earnings = apply_filters( 'pl8app_decrease_store_earnings_on_refund', true, $this );
		$decrease_customer_value = apply_filters( 'pl8app_decrease_customer_value_on_refund', true, $this );
		$decrease_purchase_count = apply_filters( 'pl8app_decrease_customer_purchase_count_on_refund', true, $this );
		...
		do_action( 'pl8app_post_refund_payment', $this );
	}
	...
	private function process_failure() {
		...
		foreach ( $discounts as $discount ) {
			pl8app_decrease_discount_usage( $discount );
		}
	...
	private function process_pending() {
		...
		// Allow extensions to filter for their own payment types, Example: Recurring Payments
		$process_pending = apply_filters( 'pl8app_should_process_pending', $process_pending, $this );

		if ( false === $process_pending ) {
			return;
		}

		$decrease_store_earnings = apply_filters( 'pl8app_decrease_store_earnings_on_pending', true, $this );
		$decrease_customer_value = apply_filters( 'pl8app_decrease_customer_value_on_pending', true, $this );
		$decrease_purchase_count = apply_filters( 'pl8app_decrease_customer_purchase_count_on_pending', true, $this );

		$this->maybe_alter_stats( $decrease_store_earnings, $decrease_customer_value, $decrease_purchase_count );
		$this->delete_sales_logs();

		$this->completed_date = false;
		$this->update_meta( '_pl8app_completed_date', '' );
	...
	private function maybe_alter_stats( $alter_store_earnings, $alter_customer_value, $alter_customer_purchase_count ) {
		...
		pl8app_undo_purchase( false, $this->ID );

		// Decrease store earnings
		if ( true === $alter_store_earnings ) {
			pl8app_decrease_total_earnings( $this->total );
		}
	...
	private function setup_completed_date() {
		...
		$date = ( $date = $this->get_meta( '_pl8app_completed_date', true ) ) ? $date : $payment->date;
	...
	private function setup_mode() {
		return $this->get_meta( '_pl8app_payment_mode' );
	}
	...
	private function setup_total() {
		...
		$amount = $this->get_meta( '_pl8app_payment_total', true );

		if ( empty( $amount ) && '0.00' != $amount ) {
			$meta   = $this->get_meta( '_pl8app_payment_meta', true );
	...
	private function setup_tax() {
		$tax = $this->get_meta( '_pl8app_payment_tax', true );
	...
	private function setup_tax_rate() {
		return $this->get_meta( '_pl8app_payment_tax_rate', true );
	}
	...
	private function setup_subtotal() {
		...
		if ( is_array( $cart_details ) ) {
			...
			$subtotal  = $this->total;
			$tax       = pl8app_use_taxes() ? $this->tax : 0;
			$subtotal -= $tax;
	...
	private function setup_currency() {
		$currency = isset( $this->payment_meta['currency'] ) ? $this->payment_meta['currency'] : apply_filters( 'pl8app_payment_currency_default', pl8app_get_currency(), $this );
		return $currency;
	}
	...
	private function setup_gateway() {
		$gateway = $this->get_meta( '_pl8app_payment_gateway', true );
		return $gateway;
	}
	...
	private function setup_transaction_id() {
		$transaction_id = $this->get_meta( '_pl8app_payment_transaction_id', true );

		if ( empty( $transaction_id ) || (int) $transaction_id === (int) $this->ID ) {

			$gateway        = $this->gateway;
			$transaction_id = apply_filters( 'pl8app_get_payment_transaction_id-' . $gateway, $this->ID );
	...
	private function setup_ip() {
		$ip = $this->get_meta( '_pl8app_payment_user_ip', true );
		return $ip;
	}
	...
	private function setup_customer_id() {
		$customer_id = $this->get_meta( '_pl8app_payment_customer_id', true );
		return $customer_id;
	}
	...
	private function setup_user_id() {
		$user_id  = $this->get_meta( '_pl8app_payment_user_id', true );
		$customer = new pl8app_Customer( $this->customer_id );
		...
		if( !empty( $customer->user_id ) && ( empty( $user_id ) || (int) $user_id !== (int) $customer->user_id ) ) {
			...
			$this->update_meta( '_pl8app_payment_user_id', $user_id );
	...
	private function setup_email() {
		$email = $this->get_meta( '_pl8app_payment_user_email', true );

		if( empty( $email ) ) {
			$email = PL8PRESS()->customers->get_column( 'email', $this->customer_id );
		}
	...
	private function setup_payment_key() {
		$key = $this->get_meta( '_pl8app_payment_purchase_key', true );
		return $key;
	}
	...
	private function setup_payment_number() {
		...
		if ( pl8app_get_option( 'enable_sequential' ) ) {
			...
			$number = $this->get_meta( '_pl8app_payment_number', true );
	 ...
	 *
	 * @since  1.0.0
	 * @return array               pl8app associated with this payment
	 */
	private function setup_menuitems() {
		$menuitems = isset( $this->payment_meta['menuitems'] ) ? maybe_unserialize( $this->payment_meta['menuitems'] ) : array();
		return $menuitems;
	}

	/**
	 * Setup the Unlimited menuitems setting
	 *
	 * @since  1.0.0
	 * @return bool If this payment has unlimited menuitems
	 */
	private function setup_has_unlimited() {
		$unlimited = (bool) $this->get_meta( '_pl8app_payment_unlimited_menuitems', true );
		return $unlimited;
	}
	...
	private function get_cart_details() {
		return apply_filters( 'pl8app_payment_cart_details', $this->cart_details, $this->ID, $this );
	}
	...
	private function get_completed_date() {
		return apply_filters( 'pl8app_payment_completed_date', $this->completed_date, $this->ID, $this );
	}
	...
	private function get_tax() {
		return apply_filters( 'pl8app_get_payment_tax', $this->tax, $this->ID, $this );
	}
	...
	private function get_subtotal() {
		return apply_filters( 'pl8app_get_payment_subtotal', $this->subtotal, $this->ID, $this );
	}
	...
	private function get_discounts() {
		return apply_filters( 'pl8app_payment_discounts', $this->discounts, $this->ID, $this );
	}
	...
	private function get_discounted_amount() {
		...
		return floatval( apply_filters( 'pl8app_payment_discounted_amount', $total - ( $fees + $tax ), $this ) );
	}
	...
	private function get_currency() {
		return apply_filters( 'pl8app_payment_currency_code', $this->currency, $this->ID, $this );
	}
	...
	private function get_gateway() {
		return apply_filters( 'pl8app_payment_gateway', $this->gateway, $this->ID, $this );
	}
	...
	private function get_transaction_id() {
		return apply_filters( 'pl8app_get_payment_transaction_id', $this->transaction_id, $this->ID, $this );
	}
	...
	private function get_ip() {
		return apply_filters( 'pl8app_payment_user_ip', $this->ip, $this->ID, $this );
	}
	...
	private function get_customer_id() {
		return apply_filters( 'pl8app_payment_customer_id', $this->customer_id, $this->ID, $this );
	}
	...
	private function get_user_id() {
		return apply_filters( 'pl8app_payment_user_id', $this->user_id, $this->ID, $this );
	}
	...
	private function get_email() {
		return apply_filters( 'pl8app_payment_user_email', $this->email, $this->ID, $this );
	}
	...
	private function get_user_info() {
		return apply_filters( 'pl8app_payment_meta_user_info', $this->user_info, $this->ID, $this );
	}
	...
	private function get_address() {
		return apply_filters( 'pl8app_payment_address', $this->address, $this->ID, $this );
	}
	...
	private function get_key() {
		return apply_filters( 'pl8app_payment_key', $this->key, $this->ID, $this );
	}
	...
	private function get_number() {
		return apply_filters( 'pl8app_payment_number', $this->number, $this->ID, $this );
	}

	/**
	 * Retrieve menuitems on payment
	 *
	 * @since 1.0
	 * @return array Payment menuitems
	 */
	private function get_menuitems() {
		return apply_filters( 'pl8app_payment_meta_menuitems', $this->menuitems, $this->ID, $this );
	}

	/**
	 * Retrieve unlimited file menuitems status
	 *
	 * @since 1.0
	 * @return bool Is unlimited
	 */
	private function get_unlimited() {
		return apply_filters( 'pl8app_payment_unlimited_menuitems', $this->unlimited, $this->ID, $this );
	}
	 ...
	 * @since 1.0
	 *
	 * @return pl8app_Customer The customer object of the existing customer or new customer.
	 */
	private function maybe_create_customer() {
		$customer = new stdClass;

		if ( did_action( 'pl8app_pre_process_purchase' ) && is_user_logged_in() ) {
		...
		if ( empty( $customer->id ) ) {
			$customer = new pl8app_Customer( $this->email );
		}
