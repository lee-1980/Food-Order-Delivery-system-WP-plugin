...
class pl8app_Customer_Query {
	...
	/**
	 * pl8app_DB_Customers instance.
	 *
	 * @access protected
	 * @since 1.0.0
	 * @var pl8app_DB_Customers
	 */
	protected $pl8app_db_customers;
	 ...
	 * }
	 */
	public function __construct( $query = '', $pl8app_db_customers = null ) {
		if ( $pl8app_db_customers ) {
			$this->pl8app_db_customers = $pl8app_db_customers;
		} else {
			$this->pl8app_db_customers = PL8PRESS()->customers;
		}

		$this->table_name       = $this->pl8app_db_customers->table_name;
		$this->meta_type        = $this->pl8app_db_customers->meta_type;
		$this->primary_key      = $this->pl8app_db_customers->primary_key;
		$this->date_key         = $this->pl8app_db_customers->date_key;
		$this->cache_group      = $this->pl8app_db_customers->cache_group;
	 ...
	 * @since 1.0.0
	 *
	 * @see pl8app_Customer_Query::__construct()
	 *
	 * @param string|array $query Array or query string of parameters. See pl8app_Customer_Query::__construct().
	...
	protected function parse_query() {
		 ...
		 * @since 1.0.0
		 *
		 * @param pl8app_Customer_Query &$this The pl8app_Customer_Query instance (passed by reference).
		 */
		do_action_ref_array( 'pl8app_parse_customer_query', array( &$this ) );
	}

	/**
	 * Retrieves a list of customers matching the query vars.
	 *
	 * Tries to use a cached value and otherwise uses `pl8app_Customer_Query::query_items()`.
	 *
	 * @access protected
	 * @since 1.0.0
	 *
	...
	protected function get_items() {
		 ...
		 * @since 1.0.0
		 *
		 * @param pl8app_Customer_Query &$this Current instance of pl8app_Customer_Query, passed by reference.
		 */
		do_action_ref_array( 'pl8app_pre_get_customers', array( &$this ) );

		// $args can include anything. Only use the args defined in the query_var_defaults to compute the key.
		$key = md5( serialize( wp_array_slice_assoc( $this->query_vars, array_keys( $this->query_var_defaults ) ) ) );

		$last_changed = $this->pl8app_db_customers->get_last_changed();
	...
	protected function set_found_items() {
		...
		if ( $this->query_vars['number'] && ! $this->query_vars['no_found_rows'] ) {
			 ...
			 * @param pl8app_Customer_Query $customer_query        The `pl8app_Customer_Query` instance.
			 */
			$found_items_query = apply_filters( 'pl8app_found_customers_query', 'SELECT FOUND_ROWS()', $this );
	...
	protected function get_allowed_orderby_keys() {
		return array_keys( $this->pl8app_db_customers->get_columns() );
	}
}
