...
class pl8app_Logging {
	...
	/**
	 * Set up the pl8app Logging Class
	 *
	 * @since  1.0.0
	 */
	...
	public function setup_log_file() {
		...
		$this->filename   = wp_hash( home_url( '/' ) ) . '-pl8app-debug.log';
	...
	/**
	 * Registers the pl8app_log Post Type
	 *
	 * @since  1.0.0
	 * @return void
	 */
	public function register_post_type() {
		/* Logs post type */
		$log_args = array(
			'labels'              => array( 'name' => __( 'Logs', 'pl8app' ) ),
			'public'              => false,
		...
		register_post_type( 'pl8app_log', $log_args );
	}
	...
	*/
	public function register_taxonomy() {
		register_taxonomy( 'pl8app_log_type', 'pl8app_log', array( 'public' => false ) );
	}
	...
	public function log_types() {
		$terms = array(
			'sale', 'file_menuitem', 'gateway_error', 'api_request'
		);

		return apply_filters( 'pl8app_log_types', $terms );
	}
	 ...
	 *
	 * @since  1.0.0
	 * @uses pl8app_Logging::insert_log()
	 ...
	 *
	 * @since  1.0.0
	 * @uses pl8app_Logging::get_connected_logs()
	...
	*/
	 ...
	 *
	 * @since  1.0.0
	 * @uses pl8app_Logging::valid_type()
	...
	function insert_log( $log_data = array(), $log_meta = array() ) {
		...
		do_action( 'pl8app_pre_insert_log', $log_data, $log_meta );
		...
		if ( $log_data['log_type'] && $this->valid_type( $log_data['log_type'] ) ) {
			wp_set_object_terms( $log_id, $log_data['log_type'], 'pl8app_log_type', false );
		}

		// Set log meta, if any
		if ( $log_id && ! empty( $log_meta ) ) {
			foreach ( (array) $log_meta as $key => $meta ) {
				update_post_meta( $log_id, '_pl8app_log_' . sanitize_key( $key ), $meta );
			}
		}

		do_action( 'pl8app_post_insert_log', $log_id, $log_data, $log_meta );
	...
	public function update_log( $log_data = array(), $log_meta = array() ) {
		...
		do_action( 'pl8app_pre_update_log', $log_data, $log_meta );
		...
		do_action( 'pl8app_post_update_log', $log_id, $log_data, $log_meta );
	}
	...
	/**
	 * Return the location of the log file that pl8app_Logging will use.
	 *
	 * Note: Do not use this file to write to the logs, please use the `pl8app_debug_log` function to do so.
	 *
	 * @since 2.9.1
	 *
	 * @return string
...
$GLOBALS['pl8app_logs'] = new pl8app_Logging();
 ...
 * @param null   $type
 *
 * @global $pl8app_logs pl8app Logs Object
 *
 * @uses pl8app_Logging::add()
 *
 * @return mixed ID of the new log entry
 */
function pl8app_record_log( $title = '', $message = '', $parent = 0, $type = null ) {
	global $pl8app_logs;
	$log = $pl8app_logs->add( $title, $message, $parent, $type );
	return $log;
}
 ...
 * @return void
 */
function pl8app_debug_log( $message = '' ) {
	global $pl8app_logs;

	if( pl8app_is_debug_mode() ) {

		$pl8app_logs->log_to_file( $message );
