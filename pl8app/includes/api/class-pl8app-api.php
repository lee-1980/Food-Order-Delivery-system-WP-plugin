 ...
 * systems, such as mobile. This class is also used in the pl8app iOS App.
 *
 * @package     pl8app
...
class pl8app_API {
	...
	/**
	 * Instance of pl8app Stats class
	 *
	 * @var object
	 * @access private
	 * @since 1.7
	...
	public function __construct() {
		...
		foreach( $this->get_versions() as $version => $class ) {
			require_once PL8_PLUGIN_DIR . 'includes/api/class-pl8app-api-' . $version . '.php';
		}

		add_action( 'init',                     array( $this, 'add_endpoint'     ) );
		add_action( 'wp',                       array( $this, 'process_query'    ), -1 );
		add_filter( 'query_vars',               array( $this, 'query_vars'       ) );
		add_action( 'pl8app_process_api_key',      array( $this, 'process_api_key'  ) );
		...
		// Setup pl8app_Stats instance
		$this->stats = new pl8app_Payment_Stats;
	...
	public function add_endpoint( $rewrite_rules ) {
		add_rewrite_endpoint( 'pl8app-api', EP_ALL );
	}
	...
	public function get_default_version() {
		...
		$version = get_option( 'pl8app_default_api_version' );

		if( defined( 'pl8app_API_VERSION' ) ) {
			$version = pl8app_API_VERSION;
		} elseif( ! $version ) {
			$version = 'v1';
		}
	...
	private function set_queried_version() {
		...
		$version = $wp_query->query_vars['pl8app-api'];
		...
		if( strpos( $version, '/' ) ) {
			...
			$wp_query->query_vars['pl8app-api'] = str_replace( $version . '/', '', $wp_query->query_vars['pl8app-api'] );
	 ...
	 * @uses pl8app_API::get_user()
	 * @uses pl8app_API::invalid_key()
	 * @uses pl8app_API::invalid_auth()
	 * @since 1.5
	 * @return bool
	 */
	private function validate_request() {
		...
		if ( ! empty( $wp_query->query_vars['pl8app-api'] ) && ( ! $this->is_public_query() || ! empty( $wp_query->query_vars['token'] ) ) ) {
		...
		} elseif ( ! empty( $wp_query->query_vars['pl8app-api'] ) && $this->is_public_query() ) {
			$this->is_valid_request = true;
	...
	private function is_public_query() {
	    ...
	    $public_modes = apply_filters( 'pl8app_api_public_query_modes', array(
	        'products'
	    ) );

	    return in_array( $wp_query->query_vars['pl8app-api'], $public_modes );
	}
	...
	public function get_user( $key = '' ) {
		...
		$user = get_transient( md5( 'pl8app_api_user_' . $key ) );

		if ( false === $user ) {
			if ( pl8app_has_upgrade_completed( 'upgrade_user_api_keys' ) ) {
				$user = $wpdb->get_var( $wpdb->prepare( "SELECT user_id FROM $wpdb->usermeta WHERE meta_key = %s LIMIT 1", $key ) );
			} else {
				$user = $wpdb->get_var( $wpdb->prepare( "SELECT user_id FROM $wpdb->usermeta WHERE meta_key = 'pl8app_user_public_key' AND meta_value = %s LIMIT 1", $key ) );
			}
			set_transient( md5( 'pl8app_api_user_' . $key ) , $user, DAY_IN_SECONDS );
		}
	...
	public function get_user_public_key( $user_id = 0 ) {
		...
		$cache_key       = md5( 'pl8app_api_user_public_key' . $user_id );
		$user_public_key = get_transient( $cache_key );

		if ( empty( $user_public_key ) ) {
			if ( pl8app_has_upgrade_completed( 'upgrade_user_api_keys' ) ) {
				$user_public_key = $wpdb->get_var( $wpdb->prepare( "SELECT meta_key FROM $wpdb->usermeta WHERE meta_value = 'pl8app_user_public_key' AND user_id = %d", $user_id ) );
			} else {
				$user_public_key = $wpdb->get_var( $wpdb->prepare( "SELECT meta_value FROM $wpdb->usermeta WHERE meta_key = 'pl8app_user_public_key' AND user_id = %d", $user_id ) );
			}
	...
	public function get_user_secret_key( $user_id = 0 ) {
		...
		$cache_key       = md5( 'pl8app_api_user_secret_key' . $user_id );
		$user_secret_key = get_transient( $cache_key );

		if ( empty( $user_secret_key ) ) {
			if ( pl8app_has_upgrade_completed( 'upgrade_user_api_keys' ) ) {
				$user_secret_key = $wpdb->get_var( $wpdb->prepare( "SELECT meta_key FROM $wpdb->usermeta WHERE meta_value = 'pl8app_user_secret_key' AND user_id = %d", $user_id ) );
			} else {
				$user_secret_key = $wpdb->get_var( $wpdb->prepare( "SELECT meta_value FROM $wpdb->usermeta WHERE meta_key = 'pl8app_user_secret_key' AND user_id = %d", $user_id ) );
			}
	 ...
	 * @access private
	 * @author pl8app
	 * @uses pl8app_API::output()
	 * @since 1.5
	 */
	private function missing_auth() {
		$error = array();
		$error['error'] = __( 'You must specify both a token and API key!', 'pl8app' );
	...
	private function invalid_auth() {
		$error = array();
		$error['error'] = __( 'Your request could not be authenticated!', 'pl8app' );
	...
	private function invalid_key() {
		$error = array();
		$error['error'] = __( 'Invalid API key!', 'pl8app' );
	...
	private function invalid_version() {
		$error = array();
		$error['error'] = __( 'Invalid API version!', 'pl8app' );
	...
	public function process_query() {
		...
		// Check for pl8app-api var. Get out if not present
		if ( empty( $wp_query->query_vars['pl8app-api'] ) ) {
			return;
		}
		...
		if( ! defined( 'pl8app_DOING_API' ) ) {
			define( 'pl8app_DOING_API', true );
		}
		...
		switch( $this->endpoint ) :
			...
			case 'file-menuitem-logs' :

				$customer = isset( $wp_query->query_vars['customer'] ) ? $wp_query->query_vars['customer']  : null;

				$data = $this->get_menuitem_logs( $customer );
		...
		$this->data = apply_filters( 'pl8app_api_output_data', $data, $this->endpoint, $this );
	...
	public function set_query_mode() {
		...
		$accepted = apply_filters( 'pl8app_api_valid_query_modes', array(
			'stats',
			'products',
			'customers',
			'sales',
		...
		$query = isset( $wp_query->query_vars['pl8app-api'] ) ? $wp_query->query_vars['pl8app-api'] : null;
		...
		if ( ! in_array( $query, $accepted ) ) {
			$error['error'] = __( 'Invalid query!', 'pl8app' );
	...
	public function per_page() {
		...
		return apply_filters( 'pl8app_api_results_per_page', $per_page );
	}
	...
	*/
	public function get_dates( $args = array() ) {
		...
		return apply_filters( 'pl8app_api_stat_dates', $dates );
	}
	...
	public function get_customers( $customer = null ) {
		...
		$customer_query = PL8PRESS()->customers->get_customers( array( 'number' => $per_page, 'offset' => $offset, $field => $customer ) );
		$customer_count = 0;
		...
		if( $customer_query ) {
			...
			foreach ( $customer_query as $customer_obj ) {
				...
				$customers['customers'][$customer_count]['stats']['total_menuitems'] = pl8app_count_file_menuitems_of_customer( $customer_obj->id );
			...
			$error['error'] = sprintf( __( 'Customer %s not found!', 'pl8app' ), $customer );
			return $error;

		} else {

			$error['error'] = __( 'No customers found!', 'pl8app' );
			return $error;

		}

		return apply_filters( 'pl8app_api_customers', $customers, $this );
	}
	...
	public function get_products( $args = array() ) {
		...
		if ( empty( $args['product'] ) ) {
			...
			if ( get_post_type( $args['product'] ) == 'menuitem' ) {
				$product_info = get_post( $args['product'] );

				$products['products'][0] = $this->get_product_data( $product_info );

			} else {
				$error['error'] = sprintf( __( 'Product %s not found!', 'pl8app' ), $args['product'] );
				return $error;
			}
		}

		return apply_filters( 'pl8app_api_products', $products, $this );
	}

	/**
	 * Given a menuitem post object, generate the data for the API output
	 *
	 * @since  2.3.9
	...
	public function get_product_data( $product_info ) {
		...
		$product['info']['tags']                         = get_the_terms( $product_info, 'menuitem_tag' );

		if( user_can( $this->user_id, 'view_shop_reports' ) || $this->override ) {
			$product['stats']['total']['sales']              = pl8app_get_menuitem_sales_stats( $product_info->ID );
			$product['stats']['total']['earnings']           = pl8app_get_menuitem_earnings_stats( $product_info->ID );
			$product['stats']['monthly_average']['sales']    = pl8app_get_average_monthly_menuitem_sales( $product_info->ID );
			$product['stats']['monthly_average']['earnings'] = pl8app_get_average_monthly_menuitem_earnings( $product_info->ID );
		}

		if ( pl8app_has_variable_prices( $product_info->ID ) ) {
			foreach ( pl8app_get_variable_prices( $product_info->ID ) as $price ) {
				$product['pricing'][ sanitize_key( $price['name'] ) ] = $price['amount'];
			}
		} else {
			$product['pricing']['amount'] = pl8app_get_menuitem_price( $product_info->ID );
		}

		if( user_can( $this->user_id, 'view_shop_sensitive_data' ) || $this->override ) {
			$product['notes'] = pl8app_get_product_notes( $product_info->ID );
		}

		return apply_filters( 'pl8app_api_products_product', $product );
	...
	public function get_stats( $args = array() ) {
		...
		if ( $args['type'] == 'sales' ) {
			if ( $args['product'] == null ) {
				if ( $args['date'] == null ) {
					...
					if( $args['enddate'] < $args['startdate'] ) {
						$error['error'] = __( 'The end date must be later than the start date!', 'pl8app' );
					}

					// Ensure both the start and end date are specified
					if ( empty( $args['startdate'] ) || empty( $args['enddate'] ) ) {
						$error['error'] = __( 'Invalid or no date range specified!', 'pl8app' );
					}

					$start_date = $dates['year'] . '-' . $dates['m_start'] . '-' . $dates['day_start'];
					$end_date = $dates['year_end'] . '-' . $dates['m_end'] . '-' . $dates['day_end'];

					$stats = PL8PRESS()->payment_stats->get_sales_by_range( 'other', true, $start_date, $end_date );
					...
					$stats = PL8PRESS()->payment_stats->get_sales_by_range( $args['date'], false, $start_date, $end_date );
					if ( $stats instanceof WP_Error ) {

						$error_message = __( 'There was an error retrieving earnings.', 'pl8app' );
				...
				$products = get_posts( array( 'post_type' => 'menuitem', 'nopaging' => true ) );
				$i = 0;
				foreach ( $products as $product_info ) {
					$sales['sales'][$i] = array( $product_info->post_name => pl8app_get_menuitem_sales_stats( $product_info->ID ) );
					$i++;
				}
			} else {
				if ( get_post_type( $args['product'] ) == 'menuitem' ) {
					$product_info = get_post( $args['product'] );
					$sales['sales'][0] = array( $product_info->post_name => pl8app_get_menuitem_sales_stats( $args['product'] ) );
				} else {
					$error['error'] = sprintf( __( 'Product %s not found!', 'pl8app' ), $args['product'] );
				}
			}
			...
			return apply_filters( 'pl8app_api_stats_sales', $sales, $this );
			...
			if ( $args['product'] == null ) {
				if ( $args['date'] == null ) {
					...
					$earnings = PL8PRESS()->payment_stats->get_earnings_by_range( 'other', true, $date_start, $date_end );
					...
					$results = PL8PRESS()->payment_stats->get_earnings_by_range( $args['date'], false, $date_start, $date_end );
					if ( $results instanceof WP_Error ) {
						...
						$earnings['earnings'][ $args['date'] ] = pl8app_format_amount( $total_earnings );
					}
				}
			} elseif ( $args['product'] == 'all' ) {
				$products = get_posts( array( 'post_type' => 'menuitem', 'nopaging' => true ) );
				...
				$i = 0;
				foreach ( $products as $product_info ) {
					$earnings['earnings'][ $i ] = array( $product_info->post_name => pl8app_get_menuitem_earnings_stats( $product_info->ID ) );
					$i++;
				}
			} else {
				if ( get_post_type( $args['product'] ) == 'menuitem' ) {
					$product_info = get_post( $args['product'] );
					$earnings['earnings'][0] = array( $product_info->post_name => pl8app_get_menuitem_earnings_stats( $args['product'] ) );
				} else {
					$error['error'] = sprintf( __( 'Product %s not found!', 'pl8app' ), $args['product'] );
				}
			}
			...
			return apply_filters( 'pl8app_api_stats_earnings', $earnings, $this );
		} elseif ( $args['type'] == 'customers' ) {
			if ( version_compare( $pl8app_version, '2.3', '<' ) || ! pl8app_has_upgrade_completed( 'upgrade_customer_payments_association' ) ) {
				global $wpdb;
				$stats = array();
				$count = $wpdb->get_col( "SELECT COUNT(DISTINCT meta_value) FROM $wpdb->postmeta WHERE meta_key = '_pl8app_payment_user_email'" );
				$stats['customers']['total_customers'] = $count[0];
				return apply_filters( 'pl8app_api_stats_customers', $stats, $this );
			} else {
				$customers = new pl8app_DB_Customers();
			...
			return apply_filters( 'pl8app_api_stats', array( 'stats' => $stats, $this ) );
		}
	}
	...
	public function get_recent_sales() {
		...
		if( isset( $wp_query->query_vars['id'] ) ) {
			$query   = array();
			$query[] = new pl8app_Payment( $wp_query->query_vars['id'] );
		} elseif( isset( $wp_query->query_vars['purchasekey'] ) ) {
			$query   = array();
			$query[] = pl8app_get_payment_by( 'key', $wp_query->query_vars['purchasekey'] );
		} elseif( isset( $wp_query->query_vars['email'] ) ) {
			$query = pl8app_get_payments( array( 'fields' => 'ids', 'meta_key' => '_pl8app_payment_user_email', 'meta_value' => $wp_query->query_vars['email'], 'number' => $this->per_page(), 'page' => $this->get_paged(), 'status' => 'publish' ) );
		} else {
			$query = pl8app_get_payments( array( 'fields' => 'ids', 'number' => $this->per_page(), 'page' => $this->get_paged(), 'status' => 'publish' ) );
		}
				...
				foreach ( $payment->cart_details as $key => $item ) {
					...
					if ( isset( $item['item_number'] ) && isset( $item['item_number']['options'] ) ) {
						...
						if ( isset( $price_options['price_id'] ) ) {
							$price_name = pl8app_get_price_option_name( $item_id, $price_options['price_id'], $payment->ID );
						}
					}
		...
		return apply_filters( 'pl8app_api_sales', $sales, $this );
	}
	...
	public function get_discounts( $discount = null ) {
		...
		if ( empty( $discount ) ) {
			...
			$paged     = $this->get_paged();
			$per_page  = $this->per_page();
			$discounts = pl8app_get_discounts( array( 'posts_per_page' => $per_page, 'paged' => $paged ) );
			$count     = 0;

			if ( empty( $discounts ) ) {
				$error['error'] = __( 'No discounts found!', 'pl8app' );
				return $error;
			}
			...
			foreach ( $discounts as $discount ) {
				...
				$discount_list['discounts'][$count]['code']                  = pl8app_get_discount_code( $discount->ID );
				$discount_list['discounts'][$count]['amount']                = pl8app_get_discount_amount( $discount->ID );
				$discount_list['discounts'][$count]['min_price']             = pl8app_get_discount_min_price( $discount->ID );
				$discount_list['discounts'][$count]['type']                  = pl8app_get_discount_type( $discount->ID );
				$discount_list['discounts'][$count]['uses']                  = pl8app_get_discount_uses( $discount->ID );
				$discount_list['discounts'][$count]['max_uses']              = pl8app_get_discount_max_uses( $discount->ID );
				$discount_list['discounts'][$count]['start_date']            = pl8app_get_discount_start_date( $discount->ID );
				$discount_list['discounts'][$count]['exp_date']              = pl8app_get_discount_expiration( $discount->ID );
				$discount_list['discounts'][$count]['status']                = $discount->post_status;
				$discount_list['discounts'][$count]['product_requirements']  = pl8app_get_discount_product_reqs( $discount->ID );
				$discount_list['discounts'][$count]['requirement_condition'] = pl8app_get_discount_product_condition( $discount->ID );
				$discount_list['discounts'][$count]['global_discount']       = pl8app_is_discount_not_global( $discount->ID );
				$discount_list['discounts'][$count]['single_use']            = pl8app_discount_is_single_use( $discount->ID );
			...
			if ( is_numeric( $discount ) && get_post( $discount ) ) {
				...
				$discount_list['discounts'][0]['code']                       = pl8app_get_discount_code( $discount );
				$discount_list['discounts'][0]['amount']                     = pl8app_get_discount_amount( $discount );
				$discount_list['discounts'][0]['min_price']                  = pl8app_get_discount_min_price( $discount );
				$discount_list['discounts'][0]['type']                       = pl8app_get_discount_type( $discount );
				$discount_list['discounts'][0]['uses']                       = pl8app_get_discount_uses( $discount );
				$discount_list['discounts'][0]['max_uses']                   = pl8app_get_discount_max_uses( $discount );
				$discount_list['discounts'][0]['start_date']                 = pl8app_get_discount_start_date( $discount );
				$discount_list['discounts'][0]['exp_date']                   = pl8app_get_discount_expiration( $discount );
				$discount_list['discounts'][0]['status']                     = get_post_field( 'post_status', $discount );
				$discount_list['discounts'][0]['product_requirements']       = pl8app_get_discount_product_reqs( $discount );
				$discount_list['discounts'][0]['requirement_condition']      = pl8app_get_discount_product_condition( $discount );
				$discount_list['discounts'][0]['global_discount']            = pl8app_is_discount_not_global( $discount );
				$discount_list['discounts'][0]['single_use']                 = pl8app_discount_is_single_use( $discount );

			} else {

				$error['error'] = sprintf( __( 'Discount %s not found!', 'pl8app' ), $discount );
				return $error;
		...
		return apply_filters( 'pl8app_api_discounts', $discount_list, $this );
	}

	/**
	 * Process Get pl8app API Request to retrieve menuitem logs
	 *
	 * @since 2.5
	 * @author pl8app
	 *
	 * @param  int $customer_id The customer ID you wish to retrieve menuitem logs for
	 * @return array            Multidimensional array of the menuitem logs
	 */
	public function get_menuitem_logs( $customer_id = 0 ) {
		global $pl8app_logs;
		...
		$logs = array();
		if ( ! $invalid_customer ) {
			$logs = $pl8app_logs->get_connected_logs( $query );
		}

		if ( empty( $logs ) ) {
			$error['error'] = __( 'No menuitem logs found!', 'pl8app' );
			return $error;
		}
		...
		foreach( $logs as $log ) {
			...
			$user_info  = isset( $log_meta['_pl8app_log_user_info'] ) ? maybe_unserialize( $log_meta['_pl8app_log_user_info'][0] ) : array();
			$payment_id = isset( $log_meta['_pl8app_log_payment_id'] ) ? $log_meta['_pl8app_log_payment_id'][0] : false;

			$payment_customer_id = pl8app_get_payment_customer_id( $payment_id );
			$payment_customer    = new pl8app_Customer( $payment_customer_id );
			$user_id             = ( $payment_customer->user_id > 0 ) ? $payment_customer->user_id : false;
			$ip                  = $log_meta['_pl8app_log_ip'][0];
			...
			$item = apply_filters( 'pl8app_api_menuitem_log_item', $item, $log, $log_meta );

			$menuitems['menuitem_logs'][] = $item;

		}

		return apply_filters( 'pl8app_api_menuitem_logs', $menuitems, $this );
	}
	...
	public function get_info() {
		...
		// Integrations
		if ( is_plugin_active( 'pl8app-commissions/pl8app-commissions.php' ) ) {
			$data['info']['integrations']['commissions'] = true;
		}

		if ( class_exists( 'pl8app_Software_Licensing' ) ) {
			$data['info']['integrations']['software_licensing'] = true;
		}

		if ( class_exists( 'pl8app_Front_End_Submissions' ) ) {
			$data['info']['integrations']['fes'] = true;
		}

		if ( class_exists( 'pl8app_Reviews' ) ) {
			$data['info']['integrations']['reviews'] = true;
		}

		if ( class_exists( 'pl8app_Recurring' ) ) {
		...
		if ( user_can( $this->user_id, 'view_shop_sensitive_data' ) ) {
			...
			$data['info']['site']['pl8app_version'] = PL8_VERSION;
		}

		$data['info']['site']['currency']            = pl8app_get_currency();
		$data['info']['site']['currency_position']   = pl8app_get_option( 'currency_position', 'before' );
		$data['info']['site']['decimal_separator']   = pl8app_get_option( 'decimal_separator', '.' );
		$data['info']['site']['thousands_separator'] = pl8app_get_option( 'thousands_separator', ',' );

		return apply_filters( 'pl8app_api_info', $data, $this );
	}
	...
	public function get_output_format() {
		...
		return apply_filters( 'pl8app_api_output_format', $format );
	}
	...
	private function log_request( $data = array() ) {
		...
		global $pl8app_logs, $wp_query;

		$query = array(
			'pl8app-api'     => $wp_query->query_vars['pl8app-api'],
	...
	public function output( $status_code = 200 ) {
		...
		do_action( 'pl8app_api_output_before', $this->data, $this, $format );

		switch ( $format ) :

			case 'xml' :

				require_once PL8_PLUGIN_DIR . 'includes/libraries/class-ArrayToXML.php';
				$arraytoxml = new ArrayToXML();
				$xml        = $arraytoxml->buildXML( $this->data, 'pl8app' );
			...
			default :
				...
				do_action( 'pl8app_api_output_' . $format, $this->data, $this );

				break;

		endswitch;

		do_action( 'pl8app_api_output_after', $this->data, $this, $format );
	...
	function user_key_field( $user ) {
		if ( ( pl8app_get_option( 'api_allow_user_keys', false ) || current_user_can( 'manage_shop_settings' ) ) && current_user_can( 'edit_user', $user->ID ) ) {
				...
				<tbody>
					<tr>
						...
						<td>
							...
							?>
							<?php if ( empty( $user->pl8app_user_public_key ) ) { ?>
								<input name="pl8app_set_api_key" type="checkbox" id="pl8app_set_api_key" value="0" />
								<span class="description"><?php _e( 'Generate API Key', 'pl8app' ); ?></span>
							<?php } else { ?>
								<strong style="display:inline-block; width: 125px;"><?php _e( 'Public key:', 'pl8app' ); ?>&nbsp;</strong><input type="text" disabled="disabled" class="regular-text" id="publickey" value="<?php echo esc_attr( $public_key ); ?>"/><br/>
								<strong style="display:inline-block; width: 125px;"><?php _e( 'Secret key:', 'pl8app' ); ?>&nbsp;</strong><input type="text" disabled="disabled" class="regular-text" id="privatekey" value="<?php echo esc_attr( $secret_key ); ?>"/><br/>
								<strong style="display:inline-block; width: 125px;"><?php _e( 'Token:', 'pl8app' ); ?>&nbsp;</strong><input type="text" disabled="disabled" class="regular-text" id="token" value="<?php echo esc_attr( $this->get_token( $user->ID ) ); ?>"/><br/>
								<input name="pl8app_set_api_key" type="checkbox" id="pl8app_set_api_key" value="0" />
								<span class="description"><label for="pl8app_set_api_key"><?php _e( 'Revoke API Keys', 'pl8app' ); ?></label></span>
							<?php } ?>
						</td>
					</tr>
				</tbody>
	...
	public function process_api_key( $args ) {
		...
		if( ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'pl8app-api-nonce' ) ) {
		...
		if ( empty( $args['user_id'] ) ) {
			wp_die( sprintf( __( 'User ID Required', 'pl8app' ), $process ), __( 'Error', 'pl8app' ), array( 'response' => 401 ) );
		}
		...
		$process    = isset( $args['pl8app_api_process'] ) ? strtolower( $args['pl8app_api_process'] ) : false;

		if( $user_id == get_current_user_id() && ! pl8app_get_option( 'allow_user_api_keys' ) && ! current_user_can( 'manage_shop_settings' ) ) {
			wp_die( sprintf( __( 'You do not have permission to %s API keys for this user', 'pl8app' ), $process ), __( 'Error', 'pl8app' ), array( 'response' => 403 ) );
		...
		switch( $process ) {
			case 'generate':
				if( $this->generate_api_key( $user_id ) ) {
					delete_transient( 'pl8app-total-api-keys' );
					wp_redirect( add_query_arg( 'pl8app-message', 'api-key-generated', 'admin.php?page=pl8app-tools&tab=api_keys' ) ); exit();
				} else {
					wp_redirect( add_query_arg( 'pl8app-message', 'api-key-failed', 'admin.php?page=pl8app-tools&tab=api_keys' ) ); exit();
				}
				break;
			case 'regenerate':
				$this->generate_api_key( $user_id, true );
				delete_transient( 'pl8app-total-api-keys' );
				wp_redirect( add_query_arg( 'pl8app-message', 'api-key-regenerated', 'admin.php?page=pl8app-tools&tab=api_keys' ) ); exit();
				break;
			case 'revoke':
				$this->revoke_api_key( $user_id );
				delete_transient( 'pl8app-total-api-keys' );
				wp_redirect( add_query_arg( 'pl8app-message', 'api-key-revoked', 'admin.php?page=pl8app-tools&tab=api_keys' ) ); exit();
				break;
			default;
				break;
		}
	...
	public function generate_api_key( $user_id = 0, $regenerate = false ) {
		...
		update_user_meta( $user_id, $new_public_key, 'pl8app_user_public_key' );
		update_user_meta( $user_id, $new_secret_key, 'pl8app_user_secret_key' );
	...
	public function revoke_api_key( $user_id = 0 ) {
		...
		$secret_key = $this->get_user_secret_key( $user_id );
		if ( ! empty( $public_key ) ) {
			delete_transient( md5( 'pl8app_api_user_' . $public_key ) );
			delete_transient( md5('pl8app_api_user_public_key' . $user_id ) );
			delete_transient( md5('pl8app_api_user_secret_key' . $user_id ) );
	...
	public function update_key( $user_id ) {
		pl8app_update_user_api_key( $user_id );
	}
	...
	private function get_default_sales_stats() {
		...
		$sales['sales']['totals']        = pl8app_get_total_sales();
	...
	private function get_default_earnings_stats() {
		...
		$earnings['earnings']['totals']        = pl8app_get_total_earnings();
	...
	public function api_key_backwards_copmat( $check, $object_id, $meta_key, $single ) {
		...
		if ( $meta_key !== 'pl8app_user_public_key' && $meta_key !== 'pl8app_user_secret_key' ) {
			return $check;
		}
		...
		switch( $meta_key ) {
			case 'pl8app_user_public_key':
				$return = PL8PRESS()->api->get_user_public_key( $object_id );
				break;
			case 'pl8app_user_secret_key':
				$return = PL8PRESS()->api->get_user_secret_key( $object_id );
				break;
		}
	...
	public function log_requests() {
		return apply_filters( 'pl8app_api_log_requests', true );
	}
