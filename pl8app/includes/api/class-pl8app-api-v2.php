 ...
 * @since  2.6
 */
class pl8app_API_V2 extends pl8app_API_V1 {
	...
	public function get_products( $args = array() ) {
		...
		if ( empty( $args['product'] ) ) {
			...
			if( ! empty( $args['tag'] ) ) {
				...
				if ( is_numeric( $args['tag'] ) ) {
					...
					$query_args['menuitem_tag'] = $args['tag'];
				}
			}
		...
		return apply_filters( 'pl8app_api_products', $products );
	}
	...
	public function get_product_data( $product_info ) {
		...
		if ( pl8app_use_skus() ) {
			$product['info']['sku'] = pl8app_get_menuitem_sku( $product['info']['id'] );
		}

		return apply_filters( 'pl8app_api_products_product_v2', $product );
	...
	public function get_customers( $args = array() ) {
		...
		$customer_query = PL8PRESS()->customers->get_customers( $args );
		$customer_count = 0;

		if( $customer_query ) {

			foreach ( $customer_query as $customer_obj ) {
				// Setup a new pl8app_Customer object so additional details are defined (like additional emails)
				$customer_obj = new pl8app_Customer( $customer_obj->id );
			...
			foreach ( $query as $payment ) {
				...
				foreach ( $payment->cart_details as $key => $item ) {
					...
					foreach ( $discount_values as $discount => $amount ) {
						...
						$item_discount = pl8app_get_cart_item_discount_amount( $item, $discount );
