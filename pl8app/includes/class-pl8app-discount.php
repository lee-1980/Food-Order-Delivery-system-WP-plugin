...
/**
 * pl8app_Discount Class
 *
 * @since 1.0.6
 */
class pl8app_Discount {
	/**
	 * Discount ID.
	 *
	 * @since 1.0.6
	...
	public function __get( $key ) {
		...
		if ( method_exists( $this, 'get_' . $key ) ) {
			...
			return new WP_Error( 'pl8app-discount-invalid-property', sprintf( __( 'Can\'t get property %s', 'restopress' ), $key ) );
		}
	}
	...
	/**
	 * Converts the instance of the pl8app_Discount object into an array for special cases.
	 *
	 * @since 1.0.6
	 *
	 * @return array pl8app_Discount object as an array.
	 */
	...
	private function find_by_code( $code = '' ) {
		...
		$discounts = pl8app_get_discounts(
			array(
				'meta_key'       => '_pl8app_discount_code',
				'meta_value'     => $code,
				'posts_per_page' => 1,
				'post_status'    => 'any',
				'fields'         => 'ids'
	...
	private function setup_discount( $discount = null ) {
		...
		if ( 'pl8app_discount' !== $discount->post_type ) {
			return false;
		}

		/**
		 * Fires before the instance of the pl8app_Discount object is set up.
		 *
		 * @since 1.0.6
		 *
		 * @param object pl8app_Discount      pl8app_Discount instance of the discount object.
		 * @param object WP_Post $discount WP_Post instance of the discount object.
		 */
		do_action( 'pl8app_pre_setup_discount', $this, $discount );
		...
		/**
		 * Fires after the instance of the pl8app_Discount object is set up. Allows extensions to add items to this object via hook.
		 *
		 * @since 1.0.6
		 *
		 * @param object pl8app_Discount      pl8app_Discount instance of the discount object.
		 * @param object WP_Post $discount WP_Post instance of the discount object.
		 */
		do_action( 'pl8app_setup_discount', $this, $discount );
	 ...
	 * @access private
	 *
	 * @return array pl8app requirements.
	 */
	...
	public function get_meta( $key = '', $single = true ) {
		$meta = get_post_meta( $this->ID, '_pl8app_discount_' . $key, $single );
		return $meta;
	}
	...
	public function update_meta( $key = '', $value = '', $prev_value = '' ) {
		...
		$key = '_pl8app_discount_' . $key;

		$value = apply_filters( 'pl8app_update_discount_meta_' . $key, $value, $this->ID );
	...
	public function get_code() {
		 ...
		 */
		return apply_filters( 'pl8app_get_discount_code', $this->code, $this->ID );
	}
	...
	public function get_status() {
		 ...
		 * @param int    $ID   Discount ID.
		 */
		return apply_filters( 'pl8app_get_discount_status', $this->status, $this->ID );
	}
	...
	public function get_status_label() {
		...
		switch( $this->status ) {
			case 'expired' :
				$label = __( 'Expired', 'pl8app' );
				break;
			case 'inactive' :
				$label = __( 'Inactive', 'pl8app' );
				break;
			case 'active' :
			default :
				$label = __( 'Active', 'pl8app' );
				break;
		}
		 ...
		 * @param int    $ID     Discount ID.
		 */
		return apply_filters( 'pl8app_get_discount_status_label', $label, $this->status, $this->ID );
	}
	...
	public function get_type() {
		 ...
		 * @param int    $ID   Discount ID.
		 */
		return apply_filters( 'pl8app_get_discount_type', $this->type, $this->ID );
	}
	...
	public function get_amount() {
		 ...
		 * @param int    $ID    Discount ID.
		 */
		return (float) apply_filters( 'pl8app_get_discount_amount', $this->amount, $this->ID );
	}
	 ...
	 * @since 1.0.6
	 *
	 * @return array IDs of required menuitems.
	 */
	public function get_product_reqs() {
		...
		/**
		 * Filters the menuitem requirements.
		 *
		 * @since 1.0.6
		 *
		 ...
		 * @param int   $ID           Discount ID.
		 */
		return (array) apply_filters( 'pl8app_get_discount_product_reqs', $this->product_reqs, $this->ID );
	}

	/**
	 * Retrieve the menuitems that are excluded from having this discount code applied.
	 *
	 * @since 1.0.6
	 *
	 * @return array IDs of excluded menuitems.
	 */
	public function get_excluded_products() {
		...
		/**
		 * Filters the excluded menuitems.
		 *
		 * @since 1.0.6
		 *
		 ...
		 * @param int   $ID                Discount ID.
		 */
		return (array) apply_filters( 'pl8app_get_discount_excluded_products', $this->excluded_products, $this->ID );
	}
	...
	public function get_start() {
		 ...
		 * @param int    $ID    Discount ID.
		 */
		return apply_filters( 'pl8app_get_discount_start', $this->start, $this->ID );
	}
	...
	public function get_expiration() {
		 ...
		 * @param int   $ID          Discount ID.
		 */
		return apply_filters( 'pl8app_get_discount_expiration', $this->expiration, $this->ID );
	}
	...
	public function get_uses() {
		 ...
		 * @param int $ID       Discount ID.
		 */
		return (int) apply_filters( 'pl8app_get_discount_uses', $this->uses, $this->ID );
	}
	...
	public function get_max_uses() {
		 ...
		 * @param int $ID       Discount ID.
		 */
		return (int) apply_filters( 'pl8app_get_discount_max_uses', $this->max_uses, $this->ID );
	}
	...
	public function get_min_price() {
		 ...
		 * @param int   $ID        Discount ID.
		 */
		return (float) apply_filters( 'pl8app_get_discount_min_price', $this->min_price, $this->ID );
	}
	...
	public function get_is_single_use() {
		 ...
		 * @param int  $ID            Discount ID.
		 */
		return (bool) apply_filters( 'pl8app_is_discount_single_use', $this->is_single_use, $this->ID );
	}
	...
	public function get_is_not_global() {
		 ...
		 * @param int  $ID            Discount ID.
		 */
		return (bool) apply_filters( 'pl8app_discount_is_not_global', $this->is_not_global, $this->ID );
	}
	...
	public function get_product_condition() {
		 ...
		 * @param int    $ID                Discount ID.
		 */
		return apply_filters( 'pl8app_discount_product_condition', $this->product_condition, $this->ID );
	}
	...
	private function insert_discount() {
		...
		$args = apply_filters( 'pl8app_insert_discount_args', array(
		...
		// Create a blank pl8app_discount post
	...
	public function save() {
		...
		if ( true == $saved ) {
			 ...
			 * @since 1.0.6
			 *
			 * @param object       Instance of pl8app_Discount object.
			 * @param string $key  Meta key.
			 */
			do_action( 'pl8app_discount_save', $this->ID, $this );
		}
	...
	public function add( $args ) {
		...
		if ( ! empty( $this->ID ) && $this->exists() ) {
			 ...
			 * @param int   $ID   Discount ID.
			 */
			$meta = apply_filters( 'pl8app_insert_discount', $meta );
			 ...
			 * @param array $meta Discount meta.
			 */
			do_action( 'pl8app_pre_insert_discount', $meta );
			 ...
			 */
			do_action( 'pl8app_post_insert_discount', $meta, $this->ID );
	...
	public function update( $args ) {
		 ...
		 * @param int   $ID   Discount ID.
		 */
		$meta = apply_filters( 'pl8app_update_discount', $meta, $this->ID );
		 ...
		 * @param int   $ID   Discount ID.
		 */
		do_action( 'pl8app_pre_update_discount', $meta, $this->ID );
		 ...
		 * @param int   $ID   Discount ID.
		 */
		do_action( 'pl8app_post_update_discount', $meta, $this->ID );
	...
	public function update_status( $new_status = 'active' ) {
		 ...
		 */
		do_action( 'pl8app_pre_update_discount_status', $this->ID, $new_status, $this->post_status );
		 ...
		 * @param string $post_status Post status.
		 */
		do_action( 'pl8app_post_update_discount_status', $this->ID, $new_status, $this->post_status );
	...
	public function is_started( $set_error = true ) {
		...
		if ( $this->start ) {
			...
			if ( $start_date < current_time( 'timestamp' ) ) {
				...
				pl8app_set_error( 'pl8app-discount-error', _x( 'Coupon code is not valid.', 'error shown when attempting to use a discount before its start date', 'pl8app' ) );
			}
		} else {
		 ...
		 * @param int  $ID     Discount ID.
		 */
		return apply_filters( 'pl8app_is_discount_started', $return, $this->ID );
	}
	...
	public function is_expired( $update = true ) {
		 ...
		 * @param int  $ID     Discount ID.
		 */
		return apply_filters( 'pl8app_is_discount_expired', $return, $this->ID );
	}
	...
	public function is_maxed_out( $set_error = true ) {
		...
		if ( $this->uses >= $this->max_uses && ! empty( $this->max_uses ) ) {
			if ( $set_error ) {
				pl8app_set_error( 'pl8app-discount-error', __( 'This discount has reached its maximum usage.', 'pl8app' ) );
			}
		 ...
		 * @param int  $ID     Discount ID.
		 */
		return apply_filters( 'pl8app_is_discount_maxed_out', $return, $this->ID );
	}
	...
	public function is_min_price_met( $set_error = true ) {
		...
		$cart_amount = pl8app_get_cart_discountable_subtotal( $this->ID );

		if ( (float) $cart_amount >= (float) $this->min_price ) {
			$return = true;
		} elseif( $set_error ) {
			pl8app_set_error( 'pl8app-discount-error', sprintf( __( 'Minimum order of %s not met.', 'pl8app' ), pl8app_currency_filter( pl8app_format_amount( $this->min_price ) ) ) );
		}
		 ...
		 * @param int  $ID     Discount ID.
		 */
		return apply_filters( 'pl8app_is_discount_min_met', $return, $this->ID );
	}
	...
	public function is_product_requirements_met( $set_error = true ) {
		...
		if ( ! $return && ! empty( $product_reqs ) ) {
			switch ( $this->product_condition ) {
				case 'all' :
					...
					foreach ( $product_reqs as $menuitem_id ) {

						if ( empty( $menuitem_id ) ) {
							continue;
						}

						if ( ! pl8app_item_in_cart( $menuitem_id ) ) {

							if ( $set_error ) {
								pl8app_set_error( 'pl8app-discount-error', __( 'The product requirements for this discount are not met.', 'pl8app' ) );
							}
		...
		if ( ! empty( $excluded_ps ) ) {
			if ( count( array_intersect( $cart_ids, $excluded_ps ) ) == count( $cart_ids ) ) {
				...
				if ( $set_error ) {
					pl8app_set_error( 'pl8app-discount-error', __( 'This discount is not valid for the cart contents.', 'pl8app' ) );
				}
			}
		}
		 ...
		 * @param string $product_condition Product condition.
		 */
		return (bool) apply_filters( 'pl8app_is_discount_products_req_met', $return, $this->ID, $this->product_condition );
	}
	...
	public function is_used( $user = '', $set_error = true ) {
			...
			if ( $payments ) {
				foreach ( $payments as $payment ) {
					...
					if ( is_array( $discounts ) ) {
						...
						if ( false !== $key ) {
							if ( $set_error ) {
								pl8app_set_error( 'pl8app-discount-error', __( 'This discount has already been redeemed.', 'pl8app' ) );
							}
		 ...
		 * @param string $user   User info.
		 */
		return apply_filters( 'pl8app_is_discount_used', $return, $this->ID, $user );
	}
	...
	public function is_valid( $user = '', $set_error = true ) {
		...
		if ( pl8app_get_cart_contents() && $this->ID ) {
			if (
			...
			pl8app_set_error( 'pl8app-discount-error', _x( 'Coupon code is not valid.', 'error for when a discount is invalid based on its configuration' , 'pl8app' ) );
		}
		 ...
		 * @param string $user   User info.
		 */
		return apply_filters( 'pl8app_is_discount_valid', $return, $this->ID, $this->code, $user );
	}
	...
	public function is_active( $update = true, $set_error = true ) {
		...
		if ( $this->exists() ) {
			if ( $this->is_expired( $update ) ) {
				if ( defined( 'DOING_AJAX' ) && $set_error ) {
					pl8app_set_error( 'pl8app-discount-error', __( 'This discount is expired.', 'pl8app' ) );
				}
			} elseif ( $this->post_status == 'active' ) {
				$return = true;
			} elseif( defined( 'DOING_AJAX' ) && $set_error ) {
				pl8app_set_error( 'pl8app-discount-error', __( 'This discount is not active.', 'pl8app' ) );
			}
		}
		 ...
		 * @param int  $ID     Discount ID.
		 */
		return apply_filters( 'pl8app_is_discount_active', $return, $this->ID );
	}
	...
	public function get_discounted_amount( $base_price ) {
		 ...
		 */
		return apply_filters( 'pl8app_discounted_amount', $amount );
	}
	...
	public function increase_usage() {
		 ...
		 * @param string $code Discount code.
		 */
		do_action( 'pl8app_discount_increase_use_count', $this->uses, $this->ID, $this->code );
	...
	public function decrease_usage() {
		 ...
		 * @param string $code Discount code.
		 */
		do_action( 'pl8app_discount_decrease_use_count', $this->uses, $this->ID, $this->code );
	...
	public function edit_url() {
		return esc_url( add_query_arg( array( 'pl8app-action' => 'edit_discount', 'discount' => $this->ID ), admin_url( 'admin.php?page=pl8app-discounts' ) ) );
	}
}
