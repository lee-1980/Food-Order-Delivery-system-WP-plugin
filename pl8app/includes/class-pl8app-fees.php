...
class pl8app_Fees {
	...
	/**
	 * Setup the pl8app Fees
	 *
	 * @since 1.0
	 */
	public function __construct() {
		add_filter( 'pl8app_payment_meta', array( $this, 'record_fees' ), 10, 2 );
	}
	...
	public function add_fee( $args = array() ) {
		...
		// If the fee is for an "item" but we passed in a menuitem id
		if( 'item' === $args['type'] && ! empty( $args['menuitem_id'] ) ) {
			unset( $args['menuitem_id'] );
			unset( $args['price_id'] );
		}

		if ( ! empty( $args['menuitem_id'] ) ) {
			$options = isset( $args['price_id'] ) ? array( 'price_id' => $args['price_id'] ) : array();
			if ( ! pl8app_item_in_cart( $args['menuitem_id'], $options ) ) {
				return false;
			}
		}
		...
		// Sanitize the amount
		$args['amount'] = pl8app_sanitize_amount( $args['amount'] );

		// Force the amount to have the proper number of decimal places.
		$args['amount'] = number_format( (float) $args['amount'], pl8app_currency_decimal_filter(), '.', '' );
		...
		// Set the fee
		$fees[ $key ] = apply_filters( 'pl8app_fees_add_fee', $args, $this );

		// Allow 3rd parties to process the fees before storing them in the session
		$fees = apply_filters( 'pl8app_fees_set_fees', $fees, $this );

		// Update fees
		PL8PRESS()->session->set( 'pl8app_cart_fees', $fees );

		do_action( 'pl8app_post_add_fee', $fees, $key, $args );
	...
	public function remove_fee( $id = '' ) {
		...
		if ( isset( $fees[ $id ] ) ) {
			...
			do_action( 'pl8app_post_remove_fee', $fees, $id );
		}
	 ...
	 * @since 1.0
	 * @param string $type Fee type, "fee" or "item"
	 * @param int $menuitem_id The menuitem ID whose fees to retrieve
	 ...
	 */
	public function get_fees( $type = 'fee', $menuitem_id = 0, $price_id = NULL ) {
		$fees = PL8PRESS()->session->get( 'pl8app_cart_fees' );

		if ( PL8PRESS()->cart->is_empty() ) {
		...
		if ( ! empty( $fees ) && ! empty( $menuitem_id ) ) {
			// Remove fees that don't belong to the specified Download
			$applied_fees = array();
			foreach ( $fees as $key => $fee ) {

				if ( empty( $fee['menuitem_id'] ) || (int) $menuitem_id !== (int) $fee['menuitem_id'] ) {
					unset( $fees[ $key ] );
				}
		...
		// Now that we've removed any fees that are for other pl8app, lets also remove any fees that don't match this price id
		if ( ! empty( $fees ) && ! empty( $menuitem_id ) && ! is_null( $price_id ) ) {
		...
		if ( ! empty( $fees ) ) {
			// Remove fees that belong to a specific menuitem but are not in the cart
			foreach ( $fees as $key => $fee ) {
				if ( empty( $fee['menuitem_id'] ) ) {
					continue;
				}

				if ( ! pl8app_item_in_cart( $fee['menuitem_id'] ) ) {
					unset( $fees[ $key ] );
				}
			}
		}
		...
		return apply_filters( 'pl8app_fees_get_fees', ! empty( $fees ) ? $fees : array(), $this );
	}
	...
	public function type_total( $type = 'fee' ) {
		...
		if ( $this->has_fees( $type ) ) {
			foreach ( $fees as $fee ) {
				$total += pl8app_sanitize_amount( $fee['amount'] );
			}
		}

		return pl8app_sanitize_amount( $total );
	}
	 ...
	 * @return float Total fee amount
	 */
	public function total( $menuitem_id = 0 ) {
		$fees  = $this->get_fees( 'all', $menuitem_id );
		$total = (float) 0.00;
