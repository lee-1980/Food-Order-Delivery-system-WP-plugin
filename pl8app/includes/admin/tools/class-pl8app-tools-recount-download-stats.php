 ...
 * @since  1.0.0
 */
class pl8app_Tools_Recount_Download_Stats extends pl8app_Batch_Export {
	...
	public function get_data() {
		...
		if ( $this->step == 1 ) {
			$this->delete_data( 'pl8app_temp_recount_menuitem_stats' );
		}

		$totals = $this->get_stored_data( 'pl8app_temp_recount_menuitem_stats' );
		...
		if ( false === $totals ) {
			...
			$this->store_data( 'pl8app_temp_recount_menuitem_stats', $totals );
		}
		...
		update_post_meta( $this->menuitem_id, '_pl8app_menuitem_sales'   , $totals['sales'] );
		update_post_meta( $this->menuitem_id, '_pl8app_menuitem_earnings', $totals['earnings'] );
	...
	public function get_percentage_complete() {
		...
		if ( $this->step == 1 ) {
			$this->delete_data( 'pl8app_recount_total_' . $this->menuitem_id );
		}

		$accepted_statuses  = apply_filters( 'pl8app_recount_accepted_statuses', array( 'publish', 'revoked' ) );
		$total   = $this->get_stored_data( 'pl8app_recount_total_' . $this->menuitem_id );
		...
		if ( false === $total ) {
			...
			$this->store_data( 'pl8app_recount_total_' . $this->menuitem_id, $total );
		}
	...
	public function process_step() {
		...
		if( $had_data ) {
			...
			$this->delete_data( 'pl8app_temp_recount_menuitem_stats' );
			$this->done    = true;
			$this->message = sprintf( __( 'Earnings and sales stats successfully recounted for %s.', 'pl8app' ), get_the_title( $this->menuitem_id ) );
			return false;
		}
	}
