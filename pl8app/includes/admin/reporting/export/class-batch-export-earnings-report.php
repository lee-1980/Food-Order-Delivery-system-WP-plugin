...
/**
 * pl8app_Earnings_Report_Export Class
 *
 * @since 1.0
 */
class pl8app_Batch_Earnings_Report_Export extends pl8app_Batch_Export {
	/**
	...
	public function headers() {
		...
		header( 'Content-Disposition: attachment; filename=' . apply_filters( 'pl8app_earnings_report_export_filename', 'pl8app-export-' . $this->export_type . '-' . date( 'm' ) . '-' . date( 'Y' ) ) . '.csv' );
		header( "Expires: 0" );
	}
	...
	public function get_csv_cols() {
		...
		$pre_status_columns = array(
			__( 'Monthly Sales Activity', 'pl8app' ),
			__( 'Gross Activity', 'pl8app' ),
		);
	...
	public function get_status_cols() {
		$status_cols        = pl8app_get_payment_statuses();
	...
	public function get_supported_statuses() {
		...
		return apply_filters( 'pl8app_export_earnings_supported_statuses', $supported_statuses );
	}
	...
	public function print_csv_cols() {
		...
		for ( $i = 1; $i <= $number_cols; $i++ ) {
			$col_data .= __( 'Order Count', 'pl8app' ) . ',';
			$col_data .= __( 'Gross Amount', 'pl8app' );
	...
	public function print_csv_rows() {
		...
		if ( $data ) {
			...
			$row_data .= '"' . pl8app_format_amount( $gross_amount ) . '",';

			foreach ( $data as $status => $status_data ) {
				$row_data .= isset( $data[ $status ]['count'] ) ? $data[ $status ]['count'] . ',' : 0 . ',';

				$column_amount = isset( $data[ $status ]['amount'] ) ? pl8app_format_amount( $data[ $status ]['amount'] ) : 0;
			...
			$completed_statuses = apply_filters( 'pl8app_export_earnings_completed_statuses', array( 'publish', 'revoked' ) );
			...
			$row_data .= $net_count . ',';
			$row_data .= '"' . pl8app_format_amount( $net_amount ) . '"';
	...
	public function get_data() {
		...
		$data = apply_filters( 'pl8app_export_get_data_' . $this->export_type, $data, $start_date, $end_date );
