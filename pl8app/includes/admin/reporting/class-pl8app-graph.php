...
class pl8app_Graph {
	...
	*/
	...
	public function get_data() {
		return apply_filters( 'pl8app_get_graph_data', $this->data, $this );
	}
	...
	public function load_scripts() {
		...
		wp_enqueue_script( 'jquery-flot', PL8_PLUGIN_URL . 'assets/js/jquery.flot' . $suffix . '.js' );

		do_action( 'pl8app_graph_load_scripts' );
	}
...
?>
		<script type="text/javascript">
			jQuery( document ).ready( function($) {
				...
				function pl8app_flot_tooltip(x, y, contents) {
					$('<div id="pl8app-flot-tooltip">' + contents + '</div>').css( {
						position: 'absolute',
						display: 'none',
						top: y + 5,
						left: x + 5,
				...
				$("#pl8app-graph-<?php echo $this->id; ?>").bind("plothover", function (event, pos, item) {
					$("#x").text(pos.x.toFixed(2));
					$("#y").text(pos.y.toFixed(2));
					if (item) {
						if (previousPoint != item.dataIndex) {
							previousPoint = item.dataIndex;
							$("#pl8app-flot-tooltip").remove();
							var x = item.datapoint[0].toFixed(2),
							y = item.datapoint[1].toFixed(2);
							if( item.series.id.includes('earnings') ) {
								if( pl8app_vars.currency_pos == 'before' ) {
									pl8app_flot_tooltip( item.pageX, item.pageY, item.series.label + ' ' + pl8app_vars.currency_sign + y );
								} else {
									pl8app_flot_tooltip( item.pageX, item.pageY, item.series.label + ' ' + y + pl8app_vars.currency_sign );
								}
							} else {
								pl8app_flot_tooltip( item.pageX, item.pageY, item.series.label + ' ' + y.replace( '.00', '' ) );
							}
						}
					} else {
						$("#pl8app-flot-tooltip").remove();
		...
		<div id="pl8app-graph-<?php echo $this->id; ?>" class="pl8app-graph" style="height: 300px;"></div>
<?php
		return ob_get_clean();
	}
	...
	public function display() {
		do_action( 'pl8app_before_graph', $this );
		echo $this->build_graph();
		do_action( 'pl8app_after_graph', $this );
	}
