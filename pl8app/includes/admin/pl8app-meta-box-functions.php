 ...
 * @param array $field
 */
function pl8app_text_input($field)
{
    ...
    switch ($data_type) {
        case 'price':
            $field['class'] .= ' pl8app_input_price';
            $field['value'] = pl8app_sanitize_amount($field['value']);
            break;
        case 'decimal':
            $field['class'] .= ' pl8app_input_decimal';
            $field['value'] = pl8app_format_localized_decimal($field['value']);
            break;
        case 'url':
            $field['class'] .= ' pl8app_input_url';
    ...
    if (!empty($field['description']) && false !== $field['desc_tip']) {
        echo pla_help_tip($field['description']);
    }
 ...
 * @param array $field
 */
function pl8app_hidden_input($field)
{
    global $thepostid, $post;
 ...
 * @param array $field
 */
function pl8app_textarea_input($field)
{
    global $thepostid, $post;
 ...
 * @param array $field
 */
function pl8app_checkbox($field)
{
    global $thepostid, $post;
...
function pl8app_select($field)
{
    ...
    ?>
    <p <?php echo pl8app_implode_html_attributes($wrapper_attributes); // WPCS: XSS ok. ?>>
        <label <?php echo pl8app_implode_html_attributes($label_attributes); // WPCS: XSS ok. ?>>
            <?php echo wp_kses_post($field['label']); ?>
        </label>
        <?php if ($tooltip) : ?>
            <?php echo pla_help_tip($tooltip); // WPCS: XSS ok. ?>
        <?php endif; ?>
        <select <?php echo pl8app_implode_html_attributes($field_attributes); // WPCS: XSS ok. ?>>
            <?php
            foreach ($field['options'] as $key => $value) {
                echo '<option value="' . esc_attr($key) . '"' . pla_selected($key, $field['value']) . '>' . esc_html($value) . '</option>';
            }
            ?>
        </select>
        <?php if ($description) : ?>
